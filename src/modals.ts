...
export class RecordListModal extends Modal {
  constructor(app: App, private plugin: VaultOrganizerPlugin, private title: string, private records: NoteRecord[]) { super(app); }
  onOpen(): void { prepare(this, "vault-organizer-list-modal"); const root = this.contentEl; root.createEl("div", { text: "VAULT FILES", cls: "vault-organizer-eyebrow" }); root.createEl("h2", { text: this.title }); root.createEl("p", { text: `${this.records.length} matching Markdown file${this.records.length === 1 ? "" : "s"}.`, cls: "vault-organizer-muted" }); const list = root.createDiv({ cls: "vault-organizer-project-files" }); for (const record of [...this.records].sort((a, b) => b.modified - a.modified)) { const row = list.createDiv({ cls: "vault-organizer-file-row" }); const open = row.createEl("button", { text: record.title, cls: "vault-organizer-file-link" }); open.onclick = () => void this.plugin.openRecord(record, record.ownership?.kind ?? record.legacyType ?? "quick-note"); row.createEl("span", { text: new Date(record.modified).toLocaleDateString(), cls: "vault-organizer-muted" }); } }
  onClose(): void { this.contentEl.empty(); }
}

export class ProjectListModal extends Modal {
  constructor(app: App, private plugin: VaultOrganizerPlugin, private projects: ProjectFolder[]) { super(app); }
  onOpen(): void { prepare(this, "vault-organizer-list-modal"); const root = this.contentEl; root.createEl("div", { text: "PROJECTS", cls: "vault-organizer-eyebrow" }); root.createEl("h2", { text: "Project workspaces" }); root.createEl("p", { text: `${this.projects.length} project folder${this.projects.length === 1 ? "" : "s"}.`, cls: "vault-organizer-muted" }); const list = root.createDiv({ cls: "vault-organizer-project-files" }); for (const project of this.projects) { const row = list.createDiv({ cls: "vault-organizer-project-row" }); const info = row.createDiv(); info.createEl("strong", { text: project.name }); info.createEl("span", { text: `${project.files} file${project.files === 1 ? "" : "s"}` }); const open = row.createEl("button", { text: "Open" }); open.onclick = () => void this.plugin.openProject(project); } }
  onClose(): void { this.contentEl.empty(); }
}
